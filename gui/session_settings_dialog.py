...
class SessionSettingsDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        self._chrome_bar = WindowChromeBar(
            "Session Settings",
            self,
            show_minimize=False,
            show_maximize=False,
        ...
        root_layout.addWidget(self._chrome_bar, 0)
        ...
        self.session_type_combo.currentTextChanged.connect(
        ...
        self.course_duration_spin.setRange(
            10, 60
        )  # Course presets: 10 to 60 minutes (step 10)
        ...
        self.course_loop_label.setStyleSheet(
        ...
        apply_glass_button_style(cancel_button)
        cancel_button.clicked.connect(self.reject)
        button_layout.addWidget(cancel_button)

        # Start Session button
        self.start_session_btn = QPushButton("Start Session")
        apply_glass_button_style(self.start_session_btn, primary=True)
    ...
    def _get_course_required_count(self, duration_minutes: int) -> Optional[int]:
        ...
        preset = next(
            (p for p in presets if p.get("duration_minutes") == duration_minutes), None
        )
        if not preset:
            return None
    ...
    def _apply_theme(self):
        ...
        dialog_rule = (
            qmw_rule.replace("QMainWindow", "QDialog", 1)
            if qmw_rule
            else """
            QDialog {
        ...
        self.setStyleSheet(dialog_rule + """
            QLabel {
                color: #f2f2f2;
            }
            QComboBox, QSpinBox {
